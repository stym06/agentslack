...
const Editor = ({
  ...
  const onTextChangeRef = useRef(onTextChangeProp)
  ...
  useLayoutEffect(() => {
    ...
    onTextChangeRef.current = onTextChangeProp
  })
  ...
  useEffect(() => {
    ...
    const notifyTextChange = () => {
      setTimeout(() => {
        ...
        if (sel && onTextChangeRef.current) {
          const fullText = quill.getText(0, sel.index)
          onTextChangeRef.current(fullText, sel.index)
        }
      }, 0)
    }
    ...
    quill.on(Quill.events.SELECTION_CHANGE, notifyTextChange)
